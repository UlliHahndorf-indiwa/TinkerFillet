...
public class StlWriterTests
{
    ...
    public void RoundTripPreservesCoordinates()
    {
        ...
        TriangleSoup original = StlReader.Read(StlFixtures.Binary([StlFixtures.SampleTriangle, second]));

        TriangleSoup reread = StlReader.Read(StlWriter.WriteBinary(original));
    ...
    public void DegenerateTriangleGetsAZeroNormalRatherThanNaN()
    {
        ...
        TriangleSoup soup = StlReader.Read(StlFixtures.Binary([degenerate]));

        (double x, double y, double z) normal = ReadFacetNormal(StlWriter.WriteBinary(soup), 0);
